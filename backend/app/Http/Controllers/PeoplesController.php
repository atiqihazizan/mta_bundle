...
class PeoplesController extends Controller
{
	...
	{
		$user = $request->user();
		$peoples = Peoples::with('healthy', 'married', 'education', 'job')->paginate(10);

		$result = tap($peoples, function ($paginatedInstance) {
			return $paginatedInstance->getCollection()->transform(function ($value) {
				$value->kesihatan = $value->healthy?->name;
				$value->perkahwinan = $value->married?->name;
				$value->pelajaran = $value->education?->name;
				$value->pekerjaan = $value->job?->name;
				unset($value->healthy);
				unset($value->married);
	...
	public function store(PeopleRequest $peopleRequest)
	{
		...
		$kariahData = $peopleRequest->validate([
			'addr_id' => 'required',
			'status' => 'required',
			'relation' => 'required',
			'tanggungan'  => 'nullable',
